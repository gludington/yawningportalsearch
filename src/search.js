    ...
    var createForm = () => {
        if (document.getElementById('upcoming-events-container')) {
            if (document.getElementById('_eventSearchForm') === null) {
                ...
                var vttStrings = /.*(roll20|foundry|zoom|discord|owlbear|fantasy grounds|fg unity|fg classic).*/i;
                var tzRegex = /(GMT)|(UTC)/;
                var campaignRegex = /(AL)|(.*Critical)|(One\s?DnD).*/;
                ...
                _evtEvents.forEach(evt => {
                    ...
                    if (!day) {
                        if (data.length === 0) {
                          var dataDivs =
                            evt.getElementsByClassName("react-tags-root");
                          if (dataDivs.length > 0) {
                            var dataDiv = dataDivs[0];
                            var dataStr = dataDiv.getAttribute("data-tags");
                            var dataTags = dataStr.split(",");
